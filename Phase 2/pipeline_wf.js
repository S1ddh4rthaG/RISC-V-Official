...
class PipelineDF {
    ...
    canDataForward(){
        let canDataForwardAll = false;
        let shouldStall = false;
        let reID_EX = [2, undefined, undefined]; //max 2 read values

        let dlist = [];
        //At the end of the cycle forward the values
        if (this.isFilled(this.MM_WB)){
            let type = this.MM_WB[this.MM_WB.length - 1];

            if (type == 'r' || type == 'i' || type == 'pT'){
                let check = this.checkReadRegs(this.MM_WB[1], this.IF_ID);
                let dpc   = check[0];
                dlist[0]  = check[1];
                ...
                if (dpc){
                    //only one dependency
                    if (dlist[0].length == 1){
                        if (dlist[0][0] == true) reID_EX = [1, this.MM_WB[2], undefined];
                        
                        canDataForwardAll = true;
                        shouldStall = false;
                    }
                    ...
                    if (dlist[0].length == 2){
                        ...
                        if (dlist[0][0] == true) reID_EX[1] = this.MM_WB[2];
                        if (dlist[0][1] == true) reID_EX[2] = this.MM_WB[2];
        ...
        if (this.isFilled(this.EX_MM)){
            ...
            if (type == 'r' || type == 'i' || type == 'pT'){
                let check = this.checkReadRegs(this.EX_MM[1], this.IF_ID);
                let dpc   = check[0];
                dlist[1]  = check[1];

                //load type get values after memory
                if (dpc && this.EX_MM[0][0] == 'l'){
                    shouldStall = true;
                    canDataForwardAll = false;
                }
                ...
                if (dpc && this.EX_MM[0][0] != 'l'){
                    ...
                    //iType
                    if (dlist[1].length == 1){
                        if (dlist[1][0] == true) reID_EX = [1, this.EX_MM[2], undefined];
                    ...
                    //Remaining types
                    if (dlist[1].length == 2){
                        if (dlist[1][0] == true) reID_EX[1] = this.EX_MM[2];
                        if (dlist[1][1] == true) reID_EX[2] = this.EX_MM[2];
        ...
        if (this.isFilled(this.ID_EX)){
            ...
            if (type == 'r' || type == 'i' || type == 'pT'){
                let check = this.checkReadRegs(this.ID_EX[1], this.IF_ID);
                let dpc = check[0];
        ...
        return [shouldStall, canDataForwardAll, reID_EX];
    }
    ...
    DisplayInfo(){
        ...
        console.log(Reverse, "Pipelined With Data Forwarding Enabled:", Reset);
    ...
    start() {
        while (this.PC < this.INST.length || this.areLatchedCleared()) {
            ...
            let DFcheck = this.canDataForward();
            let shouldStall = DFcheck[0];
            let canDFA      = DFcheck[1];
            let reID_EX     = DFcheck[2];
            ...
            // if stall removed by DataForwarding
            if (canDFA){
                if (reID_EX[0] == 1){
                    this.ID_EX[2] = reID_EX[1];
                }
                ...
                if (reID_EX[0] == 2){
                    ...
                    if (type == 'r'){
                        if (reID_EX[1] != undefined){
                            this.ID_EX[2] = reID_EX[1];
                        }
                        if (reID_EX[2] != undefined){
                            this.ID_EX[3] = reID_EX[2];
                        }
                    }else{
                        if (reID_EX[1] != undefined){
                            this.ID_EX[1] = reID_EX[1];
                        }
                        if (reID_EX[2] != undefined){
                            this.ID_EX[2] = reID_EX[2];
                        }
                    }
                }
            }
